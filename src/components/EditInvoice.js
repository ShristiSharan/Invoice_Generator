...
import { editInvoice } from '../store/invoice.slice';
...
class EditInvoice extends React.Component {
  ...
  componentDidMount(prevProps) {
    const selectedId = this.props.state.selectedInvoice;
    const selectedInvoice = this.props.state.invoices.find(invoice => invoice.id === selectedId);
    this.setState({
        ...selectedInvoice
    })
  ...
  render() {
    return (<Form onSubmit={this.updateInvoice}>
      <Row>
        <Col md={8} lg={9}>
        ...
        <Col md={4} lg={3}>
          <div className="sticky-top pt-md-3 pt-xl-4">
            <Button variant="primary" type="submit" className="d-block w-100" >Update Invoice</Button>
...
const mapDispatchToProps = (dispatch) => ({
  onEditInvoice: (invoice) => dispatch(editInvoice(invoice)),
});
